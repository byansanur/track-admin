...
public class FragmentListPetugas extends Fragment {
  ...
  private void firstLoad() {
    ...
    mApiServices.usersAll(2, limit, 0).enqueue(new Callback<UserListModel>() {
      @Override
  ...
  private void LoadMore() {
    itShouldLoadMore=false;
    mApiServices.usersAll(2,limit,offset).enqueue(new Callback<UserListModel>() {
      @Override
      public void onResponse(Call<UserListModel> call, Response<UserListModel> response) {
        if (response.isSuccessful()) {
          itShouldLoadMore = true;
